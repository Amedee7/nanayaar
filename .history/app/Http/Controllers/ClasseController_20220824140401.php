...
class ClasseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        activity("Création d\'une classe")
            ->causedBy(Auth::user())
            ->performedOn($classe)
            ->withProperties([
                'Nom de la classe'  => $classe->name,
            ])
            ->log("Création de la classe " . $classe->name);


        return response()->json(['type' => 'success', 'message' => "La classe a été créée avec succès"]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        activity("Modidification de la classe")
            ...
            ->log("Modidification de la classe " . $classe->name);
    ...
    public function destroy($id)
    {
        ...
        if ($classe) {
            $classe->delete();
            activity("Suppression de classe")
                ->causedBy(Auth::user())
                ->performedOn($classe)
                ->withProperties([
                    'Nom de la classe'  => $classe->name,
                ])
                ->log("Suppression de la classe " . $classe->name);
            return response()->json(['type' => 'success', 'message' => "La classe a été supprimée avec succès !"]);
        } else {
            return response()->json(['type' => 'error', 'message' => "Une erreur est survenue !"]);
        }
    }
