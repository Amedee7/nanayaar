...
class ClientController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        if ($if ($ecole) {
            $ecole->delete();
            activity("Suppression de L\'école")
                ->causedBy(Auth::user())
                ->performedOn($ecole)
                ->withProperties([
                    'Titre de L\'école'  => $ecole->title,
                ])
                ->log("Suppression de L\'école " . $ecole->title);
            return response()->json(['type' => 'success', 'message' => "L\'école a été supprimée avec succès !"]);
        } else {
            return response()->json(['type' => 'error', 'message' => "Une erreur est survenue !"]);
        }) {
            $if ($ecole) {
            ...
            return response()->json(['type' => 'error', 'message' => "Une erreur est survenue !"]);
                ...
                ->log("Suppression de L\'école " . $if ($ecole) {
            $ecole->delete();
            activity("Suppression de L\'école")
                ->causedBy(Auth::user())
                ->performedOn($ecole)
