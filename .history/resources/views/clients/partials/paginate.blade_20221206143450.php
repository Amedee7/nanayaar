            ...
            <tr>
                ...
                <td>
                    ...
                    <div class="mb-6">
                        <label class="flex flex-col block">
                            ...
                            <select class="form-select" id="kondisi" aria-label="select-kondisi"> name="status">
                                <option value="Accepté" @selected($item->status == 'Accepté')>Accepté</option>
                                <option value="Rejeté" @selected($item->status == 'Rejeté')>Rejeté</option>
                                <option value="Attente" @selected($item->status == 'Attente')>Attente</option>
                            </select>
                        </label>
                    </div>
                ...
                <td>
                    ...
                    <a href="{{ route('clients.details', $item->uuid) }}" title="Détails"
                        ...
                        <button onclick="deleteRecordConfirm('{{route('clients.destroy', $item->id)}}')" class="btn btn-outline-danger btn-sm"><span class="flaticon2-trash"></span></button>
                    @endif
                </td>
            </tr>
        @endforeach
