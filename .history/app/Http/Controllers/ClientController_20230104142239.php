    ...
    public function viewReport(Request $req)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            if ($req->has('exportPDF')) {
                $PDFReport = Client::whereBetween('created_at', [$startDate, $to])->get();
                // dd($PDFReport);
                $user = Auth::user();
                $configurations = Configuration::first();
            ...
            if ($req->has('exportClientAcPDF')) {
                $PDFReport = Client::where('status', 'Accepté')->whereBetween('created_at', [$startDate, $to])->get();
                $user = Auth::user();
                $configurations = Configuration::first();
            ...
            if ($req->has('exportClientAttPDF')) {
                $PDFReport = Client::where('status', 'Attente')->whereBetween('created_at', [$startDate, $to])->get();
                // $clients = Client::whereDate('created_at', DB::raw('CURDATE()'))->where('status', 'Attente')->get();
                $user = Auth::user();
                $configurations = Configuration::first();
            ...
            if ($req->has('exportClientRej')) {
                ...
                $PDFReport = Client::where('status', 'Rejeté')->whereBetween('created_at', [$startDate, $to])->get();
                $user = Auth::user();
                $configurations = Configuration::first();
