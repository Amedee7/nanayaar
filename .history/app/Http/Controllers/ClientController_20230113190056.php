        ...
        if ($request->ajax()) {
            ...
            if ($request->ajax()) {
                ...
                if (request('sts') && request('resr')) {
                    $students = Client::where('status', '=', request('sts'))
                        ->latest()
                        ->get();
                } else {
                    $students = Client::when(request('sts'), function ($query) {
                        $query->where('standard', '=', request('sts'));
                    })
                        ->when(request('ress'), function ($query) {
                            $query->where('result', '=', request('ress'));
                        })
                        ->latest()
                        ->get();
                }
