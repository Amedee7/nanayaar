...
class ClientController extends Controller
{
    ...
    public function index()
    {
        ...
        $customers = DB::table('customers')->join('users', 'users.id', '=', 'customers.created_by')
        ->select('customers.id',
            'customers.name',
            ...
            'customers.email',
            'customers.status',
            DB::raw("date_format(customers.created_at,'%d/%m/%Y à %H:%i') as created_at"),
            DB::raw("CONCAT(users.firstname,' ',users.lastname) as full_name"),
        );
        return view('clients.index', ['clients'=>$clients], ['users'=>$users]);
    }
