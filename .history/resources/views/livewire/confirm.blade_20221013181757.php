<div class="bg-white rounded-lg shadow-lg z-50 overflow-y-auto">
    <div class="py-4 text-left px-6 text-indigo-500">
        ...
        @else
            @if ($stepRented)
                <p class="text-justify">Votre séjour est maintenant préréservé. Vous devez procéder au paiement dans un délai
                    de 7 jours ou la veille du départ pour un séjour commençant avant 7 jours. Pour effectuer votre
                    réglement veuillez vous rendre sur votre compte.</p>
            @endif
            @if ($errorRented)
        ...
        @auth
            <div class="flex justify-end pt-2">
                @unless($stepRented || $errorRented)
                    <button wire:click="checkEvent"
                        class="px-4 bg-indigo-500 p-3 rounded-lg text-white hover:bg-indigo-400 mr-2 ">Je confirme</button>
                @endunless
            </div>
        @endauth
