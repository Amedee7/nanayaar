...
class VersementController extends Controller
{
    ...
    public function closeVersementPageStore(Request $request, $id)
    {
        ...
        $versement->amount_payed = $request->somme_verse;
        $versement->amount_rule = ($versement->amount_total - $request->somme_verse);
        $versement->state = 'Terminé';
        $versement->comment = $request->commentaire;
        if ($versement->amount_total - $request->somme_verse == 0) {
            $versement->status = 'Payé';
        } elseif ($versement->amount_total - $request->somme_verse > 0) {
            $versement->status = 'Paiement partiel';
        }
        $versement->save();

        $client = Client::where('id', $versement->client_id)->first();
        if ($versement->amount_total > $request->somme_verse) {
            $client->amount_credit = ($client->amount_credit + $versement->amount_rule);
        }
        $client->save();
