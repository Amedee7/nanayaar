    ...
    <div class="modal-dialog modal-dialog-centered" style="width: 1000px" role="document">
        <div class="modal-content border-0 shadow radius-1">
            ...
            <div class="modal-body modal-scroll">
                <form action="{{route('users.update', $user->id)}}" method="POST" id="o2d-edit-record-form">
                    @method('PATCH')
                    @csrf
                    <div class="form-group ">
                        <label for="">NOM</label>
                        <input name="firstname" value="{{$user->firstname}}" type="text" class="form-control " id="id-form-field-1">
                        <span class="firstname_err error text-danger"></span>
                    </div>
                    <div class="form-group ">
                        <label for="">PRENOM</label>
                        <input name="lastname" type="text" value="{{$user->lastname}}" class="form-control " id="id-form-field-1">
                        <span class="lastname_err error text-danger"></span>
                    </div>
                    <div class="form-group">
                        <label for="">TELEPHONE</label>
                        <input name="telephone" type="text" value="{{$user->telephone}}" class="form-control " id="id-form-field-1">
                        <span class="telephone_err error text-danger"></span>
                    </div>

                    <div class="form-group">
                        <label for="">ADRESSE EMAIL</label>
                        <input name="email" type="text" value="{{$user->email}}" class="form-control " id="id-form-field-1">
