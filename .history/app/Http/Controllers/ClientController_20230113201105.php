...
class ClientController extends Controller
{
    ...
    public function getStatusVersement(Request $request)
    {
        if ($request->ajax()) {
            ...
            $status_versements = Client::select('status_versement')
    ...
    public function recordsStatuses(Request $request){
        if ($request->ajax()) {

            if (request('sts') && request('sts_ver')) {
                $clients = Client::with('user')->where('status', '=', request('sts'))->where('status_versement', '=', request('sts_ver'))
                    ->latest()
                    ->get();
            } else {
                $clients = Client::with('user')->when(request('sts'), function ($query) {
                    ...
                    ->when(request('sts_ver'), function ($query) {
                        $query->where('status_versement', '=', request('sts_ver'));
                    })
                    ->latest()
                    ->get();
            }
