...
class VersementController extends Controller
{
    ...
    public function getStatusVersement(Request $request)
    {
        if ($request->ajax()) {
            ...
            $status_versements = Versement::with('client')->select('status_versement')
    ...
    public function recordStatus(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (request('sts') && request('vers')) {
                $versements = Versement::with('client.user')->where('status', '=', request('sts'))->where('status_newsversement', '=', request('vers'))
                    ->latest()
                    ->get();
            } else {
                $versements = Versement::with('client.user')->with('user')->when(request('sts'), function ($query) {
