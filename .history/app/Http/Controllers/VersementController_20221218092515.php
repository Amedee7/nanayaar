...
//pop up de modification de montant octroue
    ...
    function generateRandomIdentifier($chars)
    {
     ...
     {
         $client = Versement::where('id', $id)->first();
         if ($client) {
             $validator = Validator::make($request->all(), [
                 'somme' => 'required|integer|min:1|max:' . $client->amount_rule . ''
             ]);
             ...
             $client->amount_payed = $client->amount_payed + $request->somme;
             if ($client->amount_rule - $request->somme == 0) {
                 $client->status = 'Payé';
             }
             $client->amount_rule = $client->amount_rule - $request->somme;
             $client->save();
 
             $customer = Customer::where('id', $client->customer_id)->first();
