...
class VersementController extends Controller
{
    ...
    {
        $stocks = DB::table('ver')
            ->join('customers', 'customers.id', '=', 'ver.customer_id')
            ->join('users', 'users.id', '=', 'ver.created_by')
            ->select(
                'ver.id','ver.identifier','ver.comment',
                'ver.amount_total','amount_payed','amount_rule','state', 'ver.status',
                'customers.name','customers.telephone','users.avatar',
                DB::raw("CONCAT(users.firstname,' ',users.lastname) as full_name"),
                db::raw("date_format(ver.created_at,'%d/%m/%Y %H:%i') as created_at")
            )->where('ver.deleted_at', null);
