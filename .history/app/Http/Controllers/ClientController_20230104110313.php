        ...
        if ($req->isMethod('post'))
        {
            ...
            if ($req->has('exportPDF'))
            {
                ...
                return $pdf->download('PDF-report.pdf');
                 ...
                 {
                     $PDFReport= Client::where('status', 'Accepté')->whereBetween('created_at', [$from, $to])->get();
                     $user = Auth::user();
                     $pdf = PDF::loadView('clients.PDF_report', ['PDFReport' => $PDFReport, 
                     'user' => $user,
                     ])->setPaper('a4', 'landscape');
                     return $pdf->download('PDF-report.pdf');
                 }
            ...
            {
                $PDFReport= Client::where('status', 'Attente')->whereBetween('created_at', [$from, $to])->get();
                // $clients = Client::whereDate('created_at', DB::raw('CURDATE()'))->where('status', 'Attente')->get();
                $user = Auth::user();
                $pdf = PDF::loadView('clients.PDF_report', ['PDFReport' => $PDFReport])->setPaper('a4', 'landscape');
                return $pdf->download('PDF-report.pdf');
            }
            ...
            if ($req->has('exportClientRej'))
            {
                ...
                $PDFReport= Client::where('status', 'Rejeté')->whereBetween('created_at', [$from, $to])->get();
                $user = Auth::user();
                $pdf = PDF::loadView('clients.PDF_report', ['PDFReport' => $PDFReport, ['user' => $user]])->setPaper('a4', 'landscape');
                return $pdf->download('PDF-report.pdf');
            }  
        }
            else
