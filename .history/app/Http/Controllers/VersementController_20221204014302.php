...
class VersementController extends Controller
{
    ...
    public function modifyOutStock($id){
        $outStock = DB::table('out_stocks')->where('id', $id)->first();
        $customer = Customer::where('id', $outStock->customer_id)->first();
        $productAdded = DB::table('product_out_stocks')
            ->join('products', 'products.id', '=', 'product_out_stocks.product_id')
            ->select(
                'product_out_stocks.id','products.name','products.bottles','products.volume','products.price',
                'product_out_stocks.objects','product_out_stocks.amount_unique','product_out_stocks.amount_total_product'
            )
            ->where('enter_stock_id', $id)->get();
        //dd($productAdded);
        return view('outstock.pages.outStock', ['outStock'=>$outStock,'id'=>$id,'productAdded'=>$productAdded,'customer'=>$customer]);
    }
