                ...
                <tbody>
                    @foreach($clients as $key=>$client)
                     <tr>
                         ...
                         <td>{{ Str::limit($client->montant_demane) }}</td>
                         ...
                         <td>
                           @if($client->status==1)
                           <a href="{{ route('clients.change-status',$client->id) }}" onclick="return confirm('Are you Sure?')" class="btn btn-sm btn-success">Active</a>
                           @else
                           <a href="{{ route('clients.change-status',$client->id) }}" onclick="return confirm('Are you Sure?')" class="btn btn-sm btn-danger">Inactive</a>
                           @endif
                         </td>
                         <td>
