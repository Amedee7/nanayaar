...
Route::group(['middleware' => ['auth'], 'prefix' => 'administration'], function () {
    ...
    Route::get('//{id}/terminer', [App\Http\Controllers\StockController::class, 'closeOutStockPage'])->name('stocks.outer.termine')->middleware('permission:commande-modifier');
    Route::patch('//{id}/finish', [App\Http\Controllers\StockController::class, 'closeOutStockPageStore'])->name('stocks.outer.finish')->middleware('permission:commande-modifier');
    Route::get('//{id}/show', [App\Http\Controllers\StockController::class, 'showOutStock'])->name('stocks.outer.show')->middleware('permission:commande-lire');
    Route::get('//{id}/cancel', [App\Http\Controllers\StockController::class, 'outStockCancel'])->name('stocks.outer.cancel')->middleware('permission:commande-modifier');
    Route::get('//{id}/rule', [App\Http\Controllers\StockController::class, 'outStockAmountRule'])->name('stocks.outer.rule')->middleware('permission:commande-règlement');
    Route::patch('//{id}/rule/store', [App\Http\Controllers\StockController::class, 'outStockAmountRuleStore'])->name('stocks.outer.rule.store')->middleware('permission:commande-règlement');
    Route::delete('//{id}/delete', [App\Http\Controllers\StockController::class, 'outStockDelete'])->name('stocks.outer.delete')->middleware('permission:commande-supprimer');
