...
//pop up de modification de montant octroue
    ...
    function generateRandomIdentifier($chars)
    {
     ...
     public function outStockAmountRule($id)
     {
         $outStock = DB::table('out_stocks')->where('id', $id)->first();
         if ($outStock) {
             $customer = Customer::where('id', $outStock->customer_id)->first();
             $listProducts = Product_out_stock::where('enter_stock_id', $outStock->id)
                 ->join('products', 'products.id', '=', 'product_out_stocks.product_id')
                 ->select(
                     'products.name',
                     'products.bottles',
                     'product_out_stocks.product_id',
                     'products.price',
                     'products.volume',
                     'product_out_stocks.amount_total_product',
                     'objects'
                 )
                 ->get();
             $productsCount = Product_out_stock::where('enter_stock_id', $outStock->id)->count();
             $user = User::where('id', $outStock->created_by)->first();
             return view('outstock.modals.ruleAmount', [
                 'outStock' => $outStock, 'customer' => $customer, 'user' => $user,
                 'listProducts' => $listProducts, 'productsCount' => $productsCount
             ]);
         } else {
             return response()->json(['type' => 'error', 'message' => 'Une erreur est survenue !']);
         }
     ...
     public function outStockAmountRuleStore(Request $request, $id)
     {
         $outStock = Out_stock::where('id', $id)->first();
         if ($outStock) {
             $validator = Validator::make($request->all(), [
                 'somme' => 'required|integer|min:1|max:' . $outStock->amount_rule . ''
             ]);
             ...
             $outStock->amount_payed = $outStock->amount_payed + $request->somme;
             if ($outStock->amount_rule - $request->somme == 0) {
                 $outStock->status = 'Payé';
             }
             $outStock->amount_rule = $outStock->amount_rule - $request->somme;
             $outStock->save();
 
             $customer = Customer::where('id', $outStock->customer_id)->first();
             $customer->amount_credit = $customer->amount_credit - $request->somme;
             $customer->save();
 
             activity("Audit")
                 ->causedBy(Auth::user())
                 ->performedOn($outStock)
                 ->withProperties([
                     'Commande à regler' => $outStock,
                 ])
                 ->log("Regler somme restante'.");
