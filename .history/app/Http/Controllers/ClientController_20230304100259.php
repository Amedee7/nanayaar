...
class ClientController extends Controller
{
    ...
    public function store(Request $request, $stardate)
    {
        $validator = Validator::make($request->all(),
        
        // convertir la date stellaire en timestamp Unix
    $timestamp = strtotime($stardate);
    
    // si la conversion a échoué, la date est invalide
    if (!$timestamp) {
        return false;
    }
    
    // calculer la différence de temps entre la date stellaire et maintenant
    $now = time();
    $diff = $now - $timestamp;
    ...
    // si la différence est supérieure à 10 ans, la date est invalide
    if ($diff_years > 10) {
        return false;
    }[
            ...
            'date_delivre'          => 'required|date|date_format:Y-m-d|before:date_expire',
            'date_expire'           => 'required|date|date_format:Y-m-d|after:date_delivre',
            'adresse'               => 'required',
