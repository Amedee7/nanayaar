...
<div class="d-flex justify-content-end flex-shrink-0">
    @permission('versement-lire')
        <a onclick="editRecord('{{ route('versements.show', $versements->id) }}')" title="Détails"
    ...
    @endpermission
    @permission('versement-règlement')
        @if ($versements->status == 'Remboursement partiel')
            <a onclick="showRecord('{{ route('versements.rule', $versements->id) }}')" title="Regler la somme"
                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm mr-1">
    ...
    @permission('versement-modifier')
        @if ($versements->state == 'En cours')
            <a href="{{ route('versements.modify', $versements->id) }}" title="Modifier"
                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm mr-1">
                <span class="svg-icon svg-icon-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        fill="none">
    ...
    @permission('versement-supprimer')
        @if ($versements->state == 'En cours')
            <a onclick="cancelRecordConfirm('{{ route('versements.cancel', $versements->id) }}')" title="Annuler"
                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm mr-1">
                <span class="svg-icon svg-icon-muted svg-icon-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        fill="none">
    ...
    @permission('versement-supprimer')
        @if ($versements->state == 'Annulé')
            <a onclick="deleteRecordConfirm('{{ route('versements.delete', $versements->id) }}')"
                title="Supprimer définitivement" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm mr-1">
                <span class="svg-icon svg-icon-muted svg-icon-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                        fill="none">
                        <path
