...
function cancelRecordConfirm(link) {
    $.confirm({
        columnClass: 'medium',
        icon: 'flaticon-danger',
        title: 'Etes-vous sûr de vouloir supprimer cet enregistrement ?',
...
function storeRecordRefresh(){
    var form = $('#evote-record-form-datatable');
    $("#evote-modal-container div.modal .form-group .form-control" ).removeClass("is-invalid");
    $(".error" ).html("");
    var formNode = document.getElementById('evote-record-form-datatable');
    $.ajax({
        url: form.attr('action'),
        type: form.attr('method'),
        data: new FormData(formNode),
        ...
        success: function(result){
            ...
            switch(type){
                ...
                case 'success':
                    swal("Succès !", ''+message+'', "success");
                    setInterval(location.href = 'http://localhost/stock-cave/public/versements/sortie-stock',1000);
                    break;
                case 'error':
                    swal("Attention!", message, "error");
                    break;
        ...
        error: function(xhr, status, error) {
            var response=xhr.responseJSON;
            $.each(response, function(key, value) {
                $("#evote-modal-container div.modal .form-group."+key+" .form-control").addClass("is-invalid");
                $('.'+key+'_err').text(value);
                $('#text-load').addClass('visually-hidden');
                $('#text-fix').removeClass('visually-hidden');
                $('#btn-load').addClass('d-none');
