                    ...
                    <span style="width: 250px;">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="symbol symbol-40 symbol-light-success flex-shrink-0">
                                <span class="symbol-label font-size-h2 font-weight-bold">{{ Str::limit($item->name, 1, '') }}</span>
                            </div>
                            <div class="d-flex align-items-center">
                            <div class="ml-4">
                    ...
                    {{-- <a href="{{ route('clients.details', $item->uuid) }}" title="Détails"
                        class="btn btn-primary btn-sm">Détails</a> --}}
                            @if (request()->has('trashed'))
                                <a href="{{ route('clients.restore', $ite->id) }}" class="btn btn-success">Restore</a>
                            @else
                                <form method="POST" action="{{ route('clients.destroy', $ite->id) }}">
                                    @csrf
