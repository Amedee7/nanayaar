            ...
            <table id="o2d-datatable" class="table table-bordered table-striped bgc-grey-l4 text-100 border-b-1">
                ...
                <tbody>
                    @foreach ($versements as $key => $client)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ Str::limit($client->clientnumb_cli) }}</td>
                            <td>{{ Str::limit($client->clientname) }} {{ Str::limit($client->lastname) }}</td>
                            <td>{{ Str::limit($client->clientfirst_phone) }}</td>
                            <td>{{ Str::limit($client->clientmontant_demande) }}</td>
