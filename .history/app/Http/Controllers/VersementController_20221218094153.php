...
//pop up de modification de montant octroue
    ...
    function generateRandomIdentifier($chars)
    {
     ...
     public function versementAmountRuleStore(Request $request, $id)
     {
         ...
         if ($versement) {
             $validator = Validator::make($request->all(), [
                 'somme_verse' => 'required|integer|min:1|max:' . $versement->amount_rule . ''
             ]);
             ...
             $versement->amount_payed = $versement->amount_payed + $request->somme_verse;
             if ($versement->amount_rule - $request->somme_verse == 0) {
                 $versement->status = 'Payé';
             }
             $versement->amount_rule = $versement->amount_rule - $request->somme_verse;
             $versement->save();
