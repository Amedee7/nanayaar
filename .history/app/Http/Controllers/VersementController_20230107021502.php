...
class VersementController extends Controller
{
    ...
    public function index(Request $request, $id)
    {
        ...
        $versements = DB::table('versements')
            ...
            $results = DB::table('versements')
            ->select('nom', 'prenom', 'reste_a_payer')
            ->where('id', $id) // Récupère le client avec l'ID spécifié
            ->first();
            if (Carbon::parse(now()) > Carbon::parse(today()->subDay(2))) {
                $penalite = $results->reste_apaye * 0.01* Carbon::parse($results->fin)->diffInDays(Carbon::parse($results->debut));
                return $penalite;
