            ...
            <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4" id="o2d-datatable">
                ...
                <tbody>
                    @foreach ($versements as $key => $versement)
                        @if ($versement->fin <= now() and $client->status != 'Remboursé')
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ Str::limit($versement->client->numb_cli) }}</td>
                                <td>{{ Str::limit($versement->status) }}</td>
                                <td>{{ Str::limit($versement->client->montant_demande) }}</td>
                                <td>{{ Str::limit($versement->penalite) }}</td>
                                <td>{{ Str::limit($versement->client->name) }} {{ Str::limit($versement->lastname) }}</td>
                                <td>{{ Str::limit($versement->client->first_phone) }}</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>
