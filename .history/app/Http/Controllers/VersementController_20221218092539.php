...
//pop up de modification de montant octroue
    ...
    function generateRandomIdentifier($chars)
    {
     ...
     public function versementAmountRuleStore(Request $request, $id)
     {
         ...
         if ($versement) {
             $validator = Validator::make($request->all(), [
                 'somme' => 'required|integer|min:1|max:' . $versement->amount_rule . ''
             ]);
             ...
             $versement->amount_payed = $versement->amount_payed + $request->somme;
             if ($versement->amount_rule - $request->somme == 0) {
                 $versement->status = 'Payé';
             }
             $versement->amount_rule = $versement->amount_rule - $request->somme;
             $versement->save();
