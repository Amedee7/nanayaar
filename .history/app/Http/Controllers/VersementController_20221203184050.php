...
class VersementController extends Controller
{
    ...
    {
        $stocks = DB::table('out_stocks')
            ->join('customers', 'customers.id', '=', 'out_stocks.customer_id')
            ->join('users', 'users.id', '=', 'out_stocks.created_by')
            ->select(
                'out_stocks.id','out_stocks.identifier','out_stocks.comment',
                'out_stocks.amount_total','amount_payed','amount_rule','state', 'out_stocks.status',
                'customers.name','customers.telephone','users.avatar',
                DB::raw("CONCAT(users.firstname,' ',users.lastname) as full_name"),
                db::raw("date_format(out_stocks.created_at,'%d/%m/%Y %H:%i') as created_at")
            )->where('out_stocks.deleted_at', null);
