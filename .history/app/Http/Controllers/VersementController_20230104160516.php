    ...
    public function viewReportVersement(Request $req)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            if ($req->has('records')) {
                ...
                $records = DB::table('versements')
                    ->whereBetween('created_at', [$startDate, $endDate])
                    ->get();
                return view('versements.importVersement', ['records' => $records, 'startDate' => $startDate, 'endDate' => $endDate]);
            }
            ...
            // export PDF des versements en attente
            if ($req->has('exportClientAcPDF')) {
                $PDFReport = Versement::where('status', 'Accepté')->whereBetween('created_at', [$startDate, $endDate])->get();
                $user = Auth::user();
                $configurations = Configuration::first();

                $pdf = PDF::loadView('versements.PDF_report_Versement', [
                    'PDFReport' => $PDFReport,
                    'user' => $user,
                    'configurations' => $configurations, 'startDate' => $startDate, 'endDate' => $endDate
                ])->setPaper('a4', 'portrait');
                return $pdf->download('versement-report.pdf');
            }

            // export PDF des clients en attente
            if ($req->has('exportClientAttPDF')) {
                $PDFReport = Versement::where('status', 'Attente')->whereBetween('created_at', [$startDate, $endDate])->get();
                $user = Auth::user();
                $configurations = Configuration::first();
            ...
            if ($req->has('exportClientRej')) {
                ...
                $PDFReport = Versement::where('status', 'Rejeté')->whereBetween('created_at', [$startDate, $endDate])->get();
                $user = Auth::user();
                $configurations = Configuration::first();
