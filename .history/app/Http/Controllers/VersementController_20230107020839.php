...
class VersementController extends Controller
{
    public function datatables()
    {
        ...
        ->addColumn('penalite', function($versements) {
            if (Carbon::parse(now()) > Carbon::parse(today()->subDay(2))) {
                $penalite = $versements->reste_apaye *0.01* Carbon::parse(now())->diffInDays(Carbon::parse(today()));
                return $penalite;
    ...
    public function index(Request $request)
    {
        ...
        $versements = DB::table('versements')
            ...
            $results = Versement::select('nom', 'prenom', 'reste_a_payer')
            ->where('id', $versement_id) // Ne récupère que le client ayant l'ID spécifié
            ->first();
            if (Carbon::parse(now()) > Carbon::parse(today()->subDay(2))) {
                $penalite = CalculPenaliteService::calculPenalite($results->reste_apaye, Carbon::parse($results->fin)->diffInDays(Carbon::parse($results->debut)));
                return $penalite;
