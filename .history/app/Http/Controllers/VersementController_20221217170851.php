...
//pop up de modification de montant octroue
    ...
    public function closeOutStockPageStore(Request $request, $id)
    {
        ...
        foreach ($products as $item) {
            ...
            $stock->objects = $stock->objects - $item->objects;
            $stock->save();

            if ($item->bottles == '12') {
                $objectEmpty = new Empty_objects_stocking();
                $objectEmpty->objects = $item->objects;
                $objectEmpty->object_type = '12';
                $objectEmpty->type = 'Sortie';
                $objectEmpty->status = 'En cours';
                $objectEmpty->created_by = Auth::user()->getAuthIdentifier();
                $objectEmpty->out_stock_id = $outStock->id;
                $objectEmpty->comment = 'Caisse de type 12 en attente de retour du client ' . $customer->name . ' . Le code de la commande est ' . $outStock->identifier . '';
                $objectEmpty->save();
            } elseif ($item->bottles == '24') {
                $objectEmpty = new Empty_objects_stocking();
                $objectEmpty->objects = $item->objects;
                $objectEmpty->object_type = '24';
                $objectEmpty->type = 'Sortie';
                $objectEmpty->status = 'En cours';
                $objectEmpty->created_by = Auth::user()->getAuthIdentifier();
                $objectEmpty->out_stock_id = $outStock->id;
                $objectEmpty->comment = 'Caisse de type 24 en attente de retour du client ' . $customer->name . ' . Le code de la commande est ' . $outStock->identifier . '';
                $objectEmpty->save();
            }
        }
        ...
        activity("Audit")
            ...
            ->withProperties([
                'Sortie de stock' => $outStock,
                'Client' => $customer,
                'Produits' => $products
            ])
            ->log("Terminer une sortie de stock.");
