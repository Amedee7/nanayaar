...
class VersementController extends Controller
{
    public function datatables()
    {
        ...
        ->addColumn('colonne1', function($versements) {
            return $versements->colonne1;
        })
        ->addColumn('penality', function($versements) {
            $date1 = Carbon::now();
            $date2 = Carbon::parse($versements->limit);
            if ($date1 > $date2) {
                $difference_en_jours = $date1->diffInDays($date2);
                return $difference_en_jours * 0.01 * $versements->reste_apaye;
            } else {
                return 0;
            }
        })
