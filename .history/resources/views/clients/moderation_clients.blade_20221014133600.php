                ...
                <tbody>
                    @foreach ($clients as $key => $client)
                        <tr>
                            ...
                            <td>{{ Str::limit($client->user->firtname) }} {{ Str::limit($client->user->lastname) }}</td>

                            <td>
                                @if ($client->status == 'Accepter')
                                    <a href="{{ route('clients.change-status', $client->id) }}"
                                        onclick="return confirm('Voulez vous changer le status?')" class="btn btn-sm btn-success">Accepter</a>
                                @else
                                    <a href="{{ route('clients.change-status', $client->id) }}"
