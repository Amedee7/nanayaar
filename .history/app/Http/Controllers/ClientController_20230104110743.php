    ...
    public function viewReport(Request $req)
    {
        ...
        if ($req->isMethod('post'))
        {
            ...
            if ($req->has('search'))
            {
                ...
                return view('clients.import',['ViewsPage' => $search]);
            } 

                // export PDF des clients en attente
            if ($req->has('exportPDF'))
            {
                $PDFReport= Client::whereBetween('created_at', [$from, $to])->get();
                // dd($PDFReport);
                $user = Auth::user();
                ...
                return $pdf->download('PDF-report.pdf');
                 ...
                 {
                     $exportClientAcPDF= Client::where('status', 'Accepté')->whereBetween('created_at', [$from, $to])->get();
                     $user = Auth::user();
                     $pdf = PDF::loadView('clients.PDF_report', ['exportClientAcPDF' => $exportClientAcPDF, 
                     'user' => $user,
                     ])->setPaper('a4', 'landscape');
                     return $pdf->download('PDF-report.pdf');
                 }
            ...
            {
                $exportClientAttPDF= Client::where('status', 'Attente')->whereBetween('created_at', [$from, $to])->get();
                // $clients = Client::whereDate('created_at', DB::raw('CURDATE()'))->where('status', 'Attente')->get();
                $user = Auth::user();
                $pdf = PDF::loadView('clients.PDF_report', ['exportClientAttPDF' => $exportClientAttPDF, 
                'user' => $user,
                ])->setPaper('a4', 'landscape');
                return $pdf->download('PDF-report.pdf');
            }
            ...
            if ($req->has('exportClientRej'))
            {
                ...
                $exportClientRej= Client::where('status', 'Rejeté')->whereBetween('created_at', [$from, $to])->get();
                $user = Auth::user();
                $pdf = PDF::loadView('clients.PDF_report', ['exportClientRej' => $exportClientRej, 
                'user' => $user,
                ])->setPaper('a4', 'landscape');
                return $pdf->download('PDF-report.pdf');
            }  
