...
class VersementController extends Controller
{
    public function datatables()
    {
        ...
        ->addColumn('penalite', function($versements) {
            if (Carbon::parse(now()) > Carbon::parse(tomorow)) {
                $penalite = CalculPenaliteService::calculPenalite($versements->reste_apaye, Carbon::parse(now())->diffInDays(Carbon::parse($versements->fin)));
                return $penalite;
            } else {
                return 0;
            }
