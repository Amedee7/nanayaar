...
function updateRecord() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#sender').addClass('enabled');
            $('#sender').removeClass('disabled');
            $('#o2d-datatable').DataTable().ajax.reload( null, false ); // user paging is not reset on reload
...
//CHANGEMENT DE STATUS ACCEPTE
function accepteRecordConfirm(link) {
    $.confirm({
        columnClass: 'medium',
        icon: 'flaticon-danger',
        title: 'Etes-vous sûr de vouloir changé ce status ?',
        content: 'Il ne vous sera plus possible de revenir.',
        confirmButton: 'Oui',
        buttons: {
            tryAgain: {
                text: 'Oui, changé',
