...
class ClientController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        if ($client) {
            $client->delete();
            activity("Suppression d\'un client ")
                ->causedBy(Auth::user())
                ->performedOn($client)
                ->withProperties([
                    'Nom du client'  => $client->name,
                ])
                ->log("Suppression du client " . $client->name);
            return response()->json(['type' => 'success', 'message' => "Ce client a été supprimée avec succès !"])->redirect()->back();
        } else {
