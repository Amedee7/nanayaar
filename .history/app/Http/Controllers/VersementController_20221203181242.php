...
class VersementController extends Controller
{
    ...
    public function versementStore(Request $request){
        ...
        $verse = New Versement();
        $verse->identifier = '#S'.$this->generateRandomIdentifier(10);
        $verse->state = 'En cours';
        $verse->status = 'Non payé';
        $verse->customer_id = $request->client;
        $verse->created_by = Auth::user()->getAuthIdentifier();
        $verse->save();
