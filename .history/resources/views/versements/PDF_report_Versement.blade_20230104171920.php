        ...
        <div id="client">
            <div class="to" style="text-transform: uppercase;">
                @if (request()->has('exportDebPDF'))
                    ...
                    <em class="address"> {{ date('d/m/Y', strtotime($startDate)) }} à
    ...
    <table class="table">
        ...
        @foreach ($PDFReportVersement as $PDFReportVersements)
            <tr>
                ...
                <td>{{ number_format($PDFReportVersements['montant_octroye'], 0, '.', ' ') }} Fcfa
                <br>{{ $PDFReportVersements->status }}</td>
                <td>{{ number_format($PDFReportVersements['commission_verse'], 0, '.', ' ') }} Fcfa</td>
                <td>{{ $PDFReportVersements->status }}</td>
                <td>Dt D'Oct: {{ date('d/m/Y', strtotime($PDFReportVersements->end)) }} <br> Dt Rbsmt:{{ date('d/m/Y', strtotime($PDFReportVersements->limit)) }}</td>
