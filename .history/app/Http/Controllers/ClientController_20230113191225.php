...
class ClientController extends Controller
{
    ...
    public function getStatus(Request $request)
    {
        if ($request->ajax()) {
            ...
            $acceptes = Client::select('status')
                ->groupBy('status')
                ->get();

            return response()->json($acceptes);
        }
    }
    ...
    public function getStatusVersement(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sstatus_versement = Client::select('status_versement')
                ->groupBy('status_versement')
                ->get();

            return response()->json($sstatus_versement);
        }
    }
    ...
    public function records(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (request('sts') && request('resr')) {
                $clients = Client::with('user')->where('status', '=', request('sts'))
                    ->latest()
                    ->get();
            } else {
                $clients = Client::with('user')->when(request('sts'), function ($query) {
                    ...
                    ->when(request('resr'), function ($query) {
                        $query->where('result', '=', request('resr'));
                    })
                    ->latest()
                    ->get();
            }
