...
//pop up de modification de montant octroue
    ...
    function generateRandomIdentifier($chars)
    {
     ...
     {
         $versement = DB::table('out_stocks')->where('id', $id)->first();
         if ($versement) {
             $customer = Customer::where('id', $versement->customer_id)->first();
             $listProducts = Product_out_stock::where('enter_stock_id', $versement->id)
                 ...
                 ->get();
             $productsCount = Product_out_stock::where('enter_stock_id', $versement->id)->count();
             $user = User::where('id', $versement->created_by)->first();
             return view('outstock.modals.ruleAmount', [
                 'versement' => $versement, 'customer' => $customer, 'user' => $user,
