...
class VersementController extends Controller
{
    ...
    //STOCK OUT STORE FUNCTIONS
    public function outStockStore(Request $request){
        $validator = Validator::make($request->all(), [
        ...
        $outStock = New Versement();
        $outStock->identifier = '#S'.$this->generateRandomIdentifier(10);
        $outStock->state = 'En cours';
        $outStock->status = 'Non payé';
        $outStock->customer_id = $request->client;
        $outStock->created_by = Auth::user()->getAuthIdentifier();
        $outStock->save();
        ...
        activity("Audit")
            ...
            ->log("Initer une sortie de stock.");
