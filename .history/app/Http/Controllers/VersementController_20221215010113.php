...
//pop up de modification de montant octroue
    ...
    //ANNULER LA COMMANDE
    public function outStockCancel($id)
    {
        $outStock = Out_stock::where('id', $id)->first();
        if ($outStock) {
            $outStock->state = 'Annulé';
            $outStock->save();
            activity("Audit")
                ->causedBy(Auth::user())
                ->performedOn($outStock)
                ->withProperties([
                    'Commande annulée' => $outStock,
                ])
                ->log("Annulation d'une commande'.");
            return response()->json(['type' => 'success', 'message' => 'Commande annulée avec success !']);
        } else {
