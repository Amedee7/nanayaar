...
@section('js')
    ...
    <script type="text/javascript">
        ...
        function timeChange() {
            $.ajax({
                url: {{ route(/'clients.change-status', $client->id) }}, "{{ route('updateArticle') }}" + '/' + id,
                type: "POST",
                data: {
        ...
        function showMsg(msg, persist, disable) {
            if (disable) $('#startTime').attr('disabled', 'disabled');
            else $('#startTime').removeAttr('disabled');
            var m = $('#msg').html(msg).stop(1, 1).show();
            if (!persist) m.delay(5000).fadeOut(1000);
        }
    </script>
@endsection
