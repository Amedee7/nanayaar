...
(function (document) {
    ...
    //OUT STOCK
function outStockProductAdd(){
    var form = $('#evote-edit-form-datatable');
    $("#o2d-modal-container div.modal .form-group .form-control" ).removeClass("is-invalid");
    $(".error" ).html("");
    var formNode = document.getElementById('evote-edit-form-datatable');
    ...
    $.ajax({
        ...
        error: function(xhr, status, error) {
            var response=xhr.responseJSON;
            $.each(response, function(key, value) {
                $("#evote-edit-form-datatable div.modal .form-group."+key+" .form-control").addClass("is-invalid");
                $('.'+key+'_err').text(value);
                $('#text-load').addClass('visually-hidden');
                $('#text-fix').removeClass('visually-hidden');
                $('#btn-load').addClass('d-none');
...
function outStockProductDelete(link){
    swal({
        title: "Etes-vous sûr de vouloir supprimer cet enregistrement?",
        ...
        if (willDelete) {
            outStockProductDeleteConfirm(link);
        } else {
        }
    });
}

function outStockProductDeleteConfirm(link){
    $.ajax({
        url: link,
        type:"DELETE",
        data: {
