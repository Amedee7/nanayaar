    ...
    public function generateReport(){
        ...
        foreach ($customers as $item){
            $tmp = [
                ...
                'deconsignation12' => DB::table('empty_objects_stockings')
                    ->join('out_stocks', 'empty_objects_stockings.out_stock_id', '=', 'out_stocks.id')
                    ->join('customers', 'out_stocks.customer_id', '=', 'customers.id')
                    ->where('customers.id', $item->id)
                    ->where('empty_objects_stockings.status', 'En cours')
                    ->where('empty_objects_stockings.object_type', '12')
                    ->sum('empty_objects_stockings.objects'),
                'deconsignation24' => DB::table('empty_objects_stockings')
                    ->join('out_stocks', 'empty_objects_stockings.out_stock_id', '=', 'out_stocks.id')
                    ->join('customers', 'out_stocks.customer_id', '=', 'customers.id')
                    ->where('customers.id', $item->id)
                    ->where('empty_objects_stockings.status', 'En cours')
                    ->where('empty_objects_stockings.object_type', '24')
                    ->sum('empty_objects_stockings.objects'),
            ];
        ...
        //dd($customersState);
        $creditTotal = Out_stock::sum('amount_rule');
        $consignation12 = Empty_objects_stocking::where('status', 'En cours')->where('object_type', '12')->sum('objects');
        $consignation24 = Empty_objects_stocking::where('status', 'En cours')->where('object_type', '24')->sum('objects');

        $configurations = Configuration::first();
        $user = Auth::user();

        $pdf = PDF::loadView('state.statePDF', [
            'configurations' => $configurations, 'user' =>$user , 'creditTotal'=>$creditTotal,
            'consignation12'=>$consignation12, 'consignation24'=>$consignation24,
