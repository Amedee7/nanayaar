...
<button onclick="deleteRecordConfirm('{{ route('clients.destroy', $versements->id) }}')" title="Supprimer"
    ...
    <div class="d-flex justify-content-end flex-shrink-0">
        ...
        @permission('commande-règlement')
        @if($stocks->status == "Paiement partiel")
        <a onclick="showRecord('{{route('stocks.outer.rule', $stocks->id)}}')" title="Regler la somme" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
        ...
        @if($stocks->state == "En cours")
        <a href="{{route('stocks.outer.modify', $stocks->id)}}" title="Modifier" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
            <span class="svg-icon svg-icon-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        ...
        @permission('commande-modifier')
        @if($stocks->state == "En cours")
        <a onclick="cancelRecordConfirm('{{route('stocks.outer.cancel',$stocks->id)}}')" title="Annuler" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
            <span class="svg-icon svg-icon-muted svg-icon-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        ...
        @permission('commande-supprimer')
        @if($stocks->state == "Annulé")
            <a onclick="deleteRecordConfirm('{{route('stocks.outer.delete',$stocks->id)}}')" title="Supprimer définitivement" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
                <span class="svg-icon svg-icon-muted svg-icon-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
