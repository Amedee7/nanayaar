...
<div class="btn-group " role="group">
    ...
    <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
        ...
        @if(Auth::user()->id != $users->id)
            <a class="dropdown-item btn" onclick="statusConfirm('{{route('users.status', $users->id)}}')">
                <span class="flaticon2-{{$users->status=='Actif'?'cancel':'check-mark'}} mr-2"></span> {{$users->status=='Actif'?'Désactiver':'Activer'}}</a>
        @endif
    </div>
</div>
@endpermission