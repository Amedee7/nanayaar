...
class VersementController extends Controller
{
    ...
    public function versementPageStore(Request $request, $id)
    {
     ...
     public function modifyVersementModifierMontant($id){
        $clients = Client::join('versements', 'versements.product_id', '=', 'clients.id')
            ->select(
                'clients.id','clients.name','clients.volume','clients.bottles','clients.price',
                'stocks.objects'
            )->get();
        return view('outstock.modals.modifierMontant', ['clients'=>$clients, 'id'=>$id]);
    }

    public function modifyOutStockAddStore(Request $request, $id){
        $stock = Stock::where('product_id', $request->produit)->first();
        $productOutStockAdded = Product_out_stock::where('enter_stock_id', $id)->where('product_id', $request->produit)->sum('objects');
        $volumeDispo = $stock->objects-$productOutStockAdded;
        //dd($volumeDispo);
        $validator = Validator::make($request->all(), [
           'produit' => 'required|exists:products,id',
           'nombre' => 'required|integer|min:1|max:'.$volumeDispo.''
        ],
        [
            'nombre.max' => 'Le nombre saisi est superieur au stock restant.'
        ]);
        ...
        $product = Product::where('id',$request->produit)->first();

        $productAdd = new Product_out_stock();
