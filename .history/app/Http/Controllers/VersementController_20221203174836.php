...
class VersementController extends Controller
{
    ...
    public function closeOutStockPageStore(Request $request, $id){
        ...
        $validator = Validator::make($request->all(), [
            'somme' => 'required|integer|min:1|max:'.$versement->amount_total.'',
        ...
        $versement->amount_payed = $request->somme;
        $versement->amount_rule = ($versement->amount_total-$request->somme);
        $versement->state = 'Terminé';
        $versement->comment = $request->commentaire;
        if ($versement->amount_total-$request->somme == 0){
            $versement->status = 'Payé';
        }elseif ($versement->amount_total-$request->somme > 0){
            $versement->status = 'Paiement partiel';
        }
        $versement->save();

        $customer = Client::where('id', $versement->customer_id)->first();
        if ($versement->amount_total > $request->somme){
            $customer->amount_credit = ($customer->amount_credit+$versement->amount_rule);
        }
        $customer->save();
