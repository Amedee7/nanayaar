...
<body>
    ...
    <div id="details" class="clearfix">
        <div id="client">
            ...
            <em class="address">Periode: de {{ date('d/m/Y', strtotime($startDate)) }} à {{ date('d/m/Y', strtotime($endDate)) }}</em>
            <em class="address">Date d'extraction : {{ date('d/m/Y H:i:s', strtotime(Carbon\Carbon::now()->locale('fr_FR'))) }}</em>
    ...
    <table class="table table-dark">
        ...
        @foreach ($PDFReport as $PDFReports)
            <tr>
                ...
                <td>{{$PDFReports['numb_cli']}} <br> Tel: {{$PDFReports['first_phone']}} <br> {{$PDFReports['second_phone']}}</td>
                <td>{{number_format($item['montant_demande'],0, '.',' ')}} Fcfa</td>
                <td>{{ $PDFReports->commission_averse }}</td>
