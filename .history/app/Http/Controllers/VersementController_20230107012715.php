...
class VersementController extends Controller
{
    public function datatables()
    {
        ...
        ->addColumn('penalite', function($versements) {
            return $versements->penalite;
        })
        ->addColumn('penality', function($versements) {
           
            if (Carbon::now() < DB::raw("DATE_FORMAT(versements.fin, '%d/%m/%Y')")) {
                
                $difference_en_jours = $date1->diffInDays($date2);
                return $difference_en_jours * 0.01 * $result->montant;
            } else {
                return DB::raw('DATEDIFF(now(), fin)') * 0.01 * $versements->reste_apaye;
            }
        })
